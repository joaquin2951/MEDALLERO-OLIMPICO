...
void MedalleroPais(){
	...
	int medalleroTotal[196][4] = {0}; // col 0: IDpais, col1: cant Medallas Doradas, col2: cant Medallas platas, c3: cantMedallas Bronce
	...
	for(int i = 0; i < 196 - 1; i++){
		...
		for(int j = i + 1; j < 196; j++){
			...
			if(medalleroTotal[j][1] > mayor or //si tiene más medallas de oro
			  ...
			  (medalleroTotal[j][1] == mayor and medalleroTotal[j][2] == medalleroTotal[posMayor][2] and medalleroTotal[j][3] > medalleroTotal[i][3]))//si tiene igual medallas de oro y plata pero más de bronce
			{
	...
	while(opcionSalir != 'X'){//SE EJECUTA HASTA QUE EL USUARIO INGRESE 'X' PARA VOLVER AL MENU ANTERIOR
		...
		for(int fil = 0; fil < 196; fil++){//196 filas de la matriz
			filaVacia = medalleroTotal[fil][1] == medalleroTotal[fil][2] == medalleroTotal[fil][3] == 0; // ese pais no tiene ninguna medalla?
