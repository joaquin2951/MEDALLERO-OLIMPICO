...
int deporte_medallas[87][3]; // almacena el ID del país que ha ganado la medalla oro-plata-bronce (col 0 = oro, col 1 = plata, col 3 = bronce)
//para cada uno de los 87 deportes propuestos. 

void cargaPorDeporteIndividual(int deporteMedallas[87][3],  competidores compe[]){
...
//			"5. Carga por Categoría Deportiva" << endl;  centrar(X,Y,'Y'); //centrar menunuu
		...
		switch(opcion){
			...
			case '1': cargaPorDeporteIndividual(deporte_medallas,competencia); break;
			case '2':  system("cls"); cout << "funcionalidad en desarrollo, vuelva pronto" << endl;
	...
	cout << 
		...
		switch(opcion){
			...
			case '2': if(competenciaGenerada == true){
							cargarMedallas(); cargaDeMedallas = true;
						}else{
							gotoxy(70,25);
							SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),7*16 + 12); //error en rojo
