...
void generarTop(int mat_Top[196][2], int mat_deporteMedallas[87][3]){//generamos el TOP de los paises que más medallas ganaron
	...
	for(int fil = 0; fil < 87; fil++){//recorre categorias de deportes (filas)
		for(int col = 0; col < 3; col++){//recorre las medallas, en este caso solo hay que sumar todas, no importa de qué importancia son.
			...
			if(IDpais > 0){
				mat_Top[IDpais-1][0] = IDpais;   //le resto 1 ya que el vector se recorre de 0 a 195. En indice 0 está el pais con ID 1.
	...
	ordenarGrupo(mat_Top,196);//lo ordenamos de mayor a menor
